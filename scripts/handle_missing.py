...
def impute_forward_fill(df, time_series_cols):
    ...
    for col in time_series_cols:
        if df[col].isnull().sum() > 0:
            null_count = df[col].isnull().sum()
            df_imputed[col] = df_imputed[col].ffill() # Using ffill() instead of fillna(method='ffill') as it's deprecated in newer pandas
...
if __name__ == "__main__":
    ...
    df_original = df.copy() # keep a copy for documentation
