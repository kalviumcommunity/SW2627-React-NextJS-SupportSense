...
def remove_near_duplicates(df, key_columns, keep_strategy='most_complete'):
    ...
    if keep_strategy == 'most_complete':
        ...
        # Include include_groups=False for pandas > 2.2.0 compatibility
        try:
            df_dedup = df.groupby(key_columns, as_index=False).apply(keep_most_complete, include_groups=False).reset_index(drop=True)
        except TypeError:
...
if __name__ == "__main__":
    ...
    # Log removals (Fixing issue in prompt where df is compared to itself)
